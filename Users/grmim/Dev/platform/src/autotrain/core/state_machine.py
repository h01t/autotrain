...
The agent loop follows a simple **train → test** paradigm:

* **TRAIN** — Agent plans, changes are validated & applied, training executes.
* **TEST** — Metrics extracted, evaluated against target; next action decided.

Each cycle is one TRAIN → TEST round-trip, repeated until a terminal condition.
"""
...
class AgentState(StrEnum):
    """States of the agent loop — train → test paradigm."""

    INITIALIZING = "initializing"

    # Active cycle states
    TRAIN = "train"  # Plan → validate → apply → execute training
    TEST = "test"    # Extract metrics → evaluate → decide next
...
TRANSITIONS: dict[AgentState, set[AgentState]] = {
    AgentState.INITIALIZING: {
        AgentState.TRAIN, AgentState.FAILED,
    },
    AgentState.TRAIN: {
        AgentState.TEST, AgentState.FAILED,
        AgentState.BUDGET_EXHAUSTED, AgentState.STOPPED,
    },
    AgentState.TEST: {
        AgentState.TRAIN, AgentState.COMPLETED,
        AgentState.BUDGET_EXHAUSTED, AgentState.FAILED,
        AgentState.STOPPED,
    },
}
...
RECOVERY_ACTIONS = {
    ...
    AgentState.TRAIN: "retry_train",
    AgentState.TEST: "reevaluate_test",
}
